...
TESTS_BUCKET_URL = "s3://lakehouse-edp-dev/staging/"
...
def stream_and_merge_csv() -> None:
    ...
    pipeline = dlt.pipeline(
        ...
        dataset_name="metadata_cleaned",
    )
    # met_data contains 3 columns, where "date" column contain a date on which we want to merge
    # load all csvs in A801
    met_files = readers(bucket_url=TESTS_BUCKET_URL, file_glob="meta_data/*.csv").read_csv()
